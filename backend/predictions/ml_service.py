...
class AdherencePredictor:
    """
    ML service v9.1 — XGBoost FORCED as primary model.
    ...
    def load_dataset(self):
        ...
        noise_cols   = [c for c in numeric_cols if c not in ['adherence_risk', 'Adherence']]
        ...
        for _ in range(2):
            ...
            for col in noise_cols:
                std = df[col].std()
                if std > 0:
                    df_aug[col] = df_aug[col] + np.random.normal(0, std * 0.03, len(df_aug))
    ...
    def engineer_features(self, df):
        ...
        df['history_support']      = df['Previous_Adherence'] * (df['social_support_encoded'] / 2)
        df['comorbidity_severity'] = df['Comorbidities_Count'] * (df['severity_encoded'] + 1)
        df['risk_composite'] = (
        ...
        df['income_x_adherence']  = df['income_normalized'] * df['Previous_Adherence']
        df['severity_x_comorbid'] = (df['severity_encoded'] / 2) * (df['Comorbidities_Count'] / comorbidity_max)
        df['access_x_support']    = (df['healthcare_access_encoded'] / 2) * (df['social_support_encoded'] / 2)
        df['dosage_x_severity']   = df['dosage_normalized'] * (df['severity_encoded'] / 2 + 0.001)
        df['age_x_comorbid']      = (df['Age'] / 100) * (df['Comorbidities_Count'] / comorbidity_max)
        df['mental_x_income']     = (df['mental_health_encoded'] / mental_max) * df['income_normalized']
        df['prev_adh_x_severity'] = df['Previous_Adherence'] * (1 - df['severity_encoded'] / 2)
        df['insurance_x_income']  = df['Insurance_Coverage'] * df['income_normalized']
    ...
    def prepare_features(self, df):
        ...
        if SMOTE_AVAILABLE:
            ...
            try:
                X_selected, y = SMOTE(random_state=42, k_neighbors=5).fit_resample(X_selected, y)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # TRAINING — XGBoost FORCED as primary
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def train_models(self):
        ...
        print("📊 LEADERBOARD")
        print("=" * 70)
        print(f"{'Rank':<5} {'Model':<25} {'Accuracy':>10} {'F1':>10} {'CV F1':>10}")
        print("-" * 65)
        for i, r in enumerate(results, 1):
            flag = " 🏆" if r['model_name'] == 'XGBoost' else (" ⭐" if r['f1_score'] >= 0.90 else "")
    ...
    def load_model(self, model_name=None):
        if model_name is None:
            ...
            # Prefer XGBoost model if available
    ...
    def predict(self, input_data):
        ...
        if self.all_engineered_cols:
            df_row = df_row[self.all_engineered_cols].fillna(0)
        X_raw = df_row.values

        X_rfe    = self.rfe_selector.transform(X_raw) if self.rfe_selector else X_raw
    ...
    def _generate_recommendation(self, risk_level, age, num_medicines, treatment_duration_days):
        recommendations = {
            'low': [
    ...
    def get_dataset_statistics(self):
        ...
        return {
            ...
            'dataset_source':        'Kaggle: patient_adherence_dataset.csv (5k real + augmented)',
        }
